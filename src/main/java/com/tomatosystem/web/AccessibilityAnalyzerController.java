...
import com.tomatosystem.service.AccessibilityAnalyzerService;
...
@RestController
@RequestMapping("/figma/accessibility")
public class AccessibilityAnalyzerController {
    ...
    public ResponseEntity<String> analyzeAccessibility(HttpServletRequest request, HttpServletResponse response, DataRequest dataRequest) {
        ...
        try {
            ...
            accessibilityAnalyzerService.analyzeAccessibility(figmaJson, outputPath);

            return ResponseEntity.ok("Accessibility analysis completed. Results saved in: " + outputPath);

        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error during accessibility analysis: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<List<Map<String, String>>> listReports() {
        try {
            ...
            File[] reportFiles = dir.listFiles((d, name) -> 
                name.endsWith(".json") || 
                name.endsWith(".html")
            );
    ...
    public ResponseEntity<String> viewReport(@PathVariable String date, @PathVariable String fileName) {
        try {
