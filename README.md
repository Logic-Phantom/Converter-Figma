...
## 목차
0. [한눈에 보기](#0-한눈에-보기)
1. [동작 원리 (결정적 파이프라인)](#1-동작-원리-결정적-파이프라인)
2. [변환 예시](#2-변환-예시)
3. [AI 보조 변환 (v2.1, 선택)](#3-ai-보조-변환-v21-선택)
4. [API 엔드포인트와 출력](#4-api-엔드포인트와-출력)
5. [설정](#5-설정)
6. [2025~2026 Figma API 변경과 반영 내역](#6-20252026-figma-api-변경과-반영-내역)
7. [검증 방법](#7-검증-방법)
8. [폴더 구조](#8-폴더-구조)
9. [작업 내역](#9-작업-내역)
10. [문제 해결](#10-문제-해결)
...
| 항목 | v1.x (좌표 복사) | v2.0 (템플릿 기반) | v2.1 (AI 보조, 선택) |
|---|---|---|---|
| 레이아웃 | 모든 요소를 `cl:xylayout` 절대좌표로 | 가장 유사한 템플릿(P1~P8)의 formlayout/verticallayout 뼈대 | v2.0 과 동일 |
| 컨트롤 판별 | 레이어 이름에 "input"/"radio" 등이 있는지 | INSTANCE 의 **원본 컴포넌트셋/컴포넌트명** + 보이는 내부 아이콘 | 애매한 건만 AI 재검토 |
| 버튼 캡션 | 레이어명/직계 TEXT (대부분 "Button") | `componentProperties` TEXT → 보이는 텍스트 | 동일 |
| 표(그리드) | 이름이 정확히 `table` 일 때 5열 빈 그리드 | 헤더/폭/행선택/rowindex/셀 편집기/페이징/엑셀 추출 | 셀 편집기 오판 교정 |
| 컬럼 코드 | 없음 | 사전 40개 + 디자인의 대문자 식별자, 나머지는 `COL1…` | 사전 → 캐시 → AI 로 의미 있는 코드 |
| 숨김 레이어 | 그대로 출력 | `visible:false` 제외 | 동일 |
| UDC | `udcComAppHeader` 만 | AppHeader, GridTitle(ctrl·엑셀), FormTitle, **GridCudBtns** | 동일 |
| 결과 검증 | 없음 | `ClxValidator` + e6-compiler | 검증 실패 시 AI 수리 후 실패하면 규칙 결과로 복귀 |
| 저장 경로 | `C:\Users\LCM\...` 하드코딩 | 워크스페이스에서 프로젝트 루트 자동 탐색 | 동일 |
| OAuth | `file_read` (2025-11-17 폐지 → 로그인 실패) | `file_content:read projects:read`, Basic 인증, state 검증 | 동일 |
...
5. (선택) AI 보조를 켜려면 §3 참고 — 키 설정 후 `/designAi/preview.do` 로 먼저 확인하고 `/designAi/convert.do` 사용.
...
Figma JSON 은 텍스트·컴포넌트·좌표가 정확하므로 **이 경로는 결정적**이다(같은 입력 → 같은 결과, 외부 호출 없음).
...
**설계 문서: [docs/ai-architecture.md](docs/ai-architecture.md)** — 후보 구성 비교(왜 "AI가 CLX 생성"을 버렸는지), 안전장치, 비용/한도, 검증 결과.

§1 의 결정적 파이프라인은 그대로 두고, 애매한 판정과 이름 짓기만 AI 에게 물어보는 경로를 **별도 패키지(`figma/ai`)·별도 엔드포인트(`/designAi/*`)** 로 추가했다.
`figma.ai.provider=none`(기본)이면 AI 호출이 전혀 없고 결과도 v2.0 과 동일하다.

```
Figma JSON → (규칙) UI-IR ─┬─ ① 컬럼 코드: 사전 → 캐시 → AI (LabelCodeNamer)
                           ├─ ② 구조 검토: AI 는 화이트리스트 patch 만 제안 (UiIrCritic, +선택: 화면 렌더 PNG)
                           └─ ③ 수리: 검증 실패 시 최소 수정 요청(최대 2회) → 그래도 실패하면 규칙 결과로 복귀
                                        ↓
                        기존 UiIrParser → 템플릿 선택 → ClxGenerator → ClxValidator → 저장
```
...
| 역할 | 클래스 | 내용 |
|---|---|---|
| ① 컬럼 코드 | `LabelCodeNamer` | 그리드 헤더 → 데이터셋 컬럼코드. 디자인의 대문자 식별자 → 사전(`ColumnNames`) → 파일 캐시 → 남은 것만 AI 1회. 결과는 캐시에 저장되어 **반복 변환은 API 0회** |
| ② 구조 검토 | `UiIrCritic` | 압축 UI-IR(1~4KB, 선택적으로 렌더 PNG)을 보내 조회/상세 오판, 컨트롤·셀 편집기 오판, 장식 영역 등을 교정 |
| ③ 수리 | `UiIrCritic.repair` | `ClxValidator` 오류 메시지를 주고 최소 수정 patch 를 요청, 최대 2회 재생성 |

### 3.2 제공자 (모두 무료)

| provider | 비용 | 비고 |
|---|---|---|
| `none` (기본) | 0 | AI 사용 안 함 = v2.0 |
| `gemini` | 무료 등급 | `gemini-3.5-flash-lite`(기본). **무료 등급은 입력이 Google 제품 개선에 사용** → 기밀 디자인 금지 |
| `ollama` | 0 | 사내 로컬 모델(`qwen3:4b` 등), 외부 전송 없음 |

호출량: 화면당 1회(+수리 시 최대 2회, 컬럼 코드 1회). 429/키 없음/타임아웃은 **조용히 폴백**하고 경고만 남긴다.

### 3.3 안전장치 — AI 는 CLX 를 만들지 않는다

AI 출력은 **UI-IR 에 대한 허용된 patch 목록**뿐이다(`UiIrPatch`).

| 허용 op | 비고 |
|---|---|
| `screen.name`, `region.title` | 이름/제목 |
| `region.type` | search↔form, description↔sectionTitle **안에서만** (grid→form 등은 거부) |
| `region.drop` | description/sectionTitle/buttons 만, 1회 |
| `field.label`, `field.component`, `field.required` | component 는 지원 목록 안에서만 |
| `column.header`, `column.editor`, `column.name` | editor 는 지원 목록, name 은 대문자 컬럼코드 형식 |

영역 추가·순서 변경·좌표 변경·CLX 편집은 불가. 인덱스 범위 밖, 모르는 op, 형식 오류는 거부된다.
적용 후 **UI-IR 재파싱 + CLX 검증**을 통과해야 하며, 실패하면 AI 보정을 버리고 규칙 결과로 되돌린다.
적용/거부 내역은 응답과 `generated/ui-ir/<프레임>.ui-ir.json` 의 `ai` 에 남아 감사할 수 있다.

### 3.4 켜는 법

```powershell
setx FIGMA_AI_GEMINI_APIKEY "발급받은_키"      # 또는 사내: figma.ai.provider=ollama
# application.properties: figma.ai.provider=gemini
```
1. `/designAi/status.do` 로 provider·키 인식 확인
2. `/designAi/preview.do?url=<Figma 링크>` 로 **무엇을 고치려 하는지** 먼저 확인(파일 안 씀)
3. `/designAi/convert.do?url=<Figma 링크>` 로 생성
...
| `GET /designAi/convert.do` | convertDirect.do 와 동일 | **AI 보조** 변환 (§3) |
...
`src/main/resources/application.properties` (Figma·AI) 와 `src/main/resources/exconverter/exconverter.properties` (템플릿 엔진).
...
| `exconverter.generated.root` | `generated` | UI-IR/캐시 파일 루트 |
...
| `figma.ai.gemini.model` / `.url` | `gemini-3.5-flash-lite` / `https://generativelanguage.googleapis.com` | 무료 한도가 큰 모델 |
| `figma.ai.ollama.url` / `.model` | `http://127.0.0.1:11434` / `qwen3:4b` | 사내 로컬 |
| `figma.ai.screenshot` / `.maxSide` | `false` / `1024` | 화면 렌더 PNG 동봉(이미지 토큰 사용, Figma `/v1/images` 1회) |
| `figma.ai.naming` | `true` | 컬럼 코드 이름 짓기(사전·캐시는 AI 없이도 동작) |
...
## 6. 2025~2026 Figma API 변경과 반영 내역
...
- **AI(LLM) 로 CLX 직접 생성 / UI-IR 전체 생성**: 결과가 매번 달라지고 검증이 불가능하다. v2.1 에서도 AI 는 patch 만 낸다(§3.3, docs/ai-architecture.md §2).
...
| 항목 | 결과 (2026-09-19 ~ 09-20) |
|---|---|
| 규칙 경로 | `clx-src/json` 47개 응답 중 **46개 화면 생성·ClxValidator 통과**. 1개(`2025-05-12`)는 `10건` 텍스트 조각뿐이라 "변환할 요소를 찾지 못했습니다" 가 정상 |
| eXBuilder 컴파일 | 46개 CLX → e6-compiler **BUILD SUCCESS**, `.clx.js` 46개. UDC 속성(`udc.grid = linker.grid_1`, `ctrl`, `showExportExcel`, `visible*Button`) 확인 |
| 대표 화면 | `우리보건소 등록카드` → P1-4, `프로젝트관리` → P5-2 |
...
│       ├── UiIrCritic          구조 검토·수리 프롬프트와 폴백
│       ├── LabelCodeNamer      라벨 → 컬럼코드 (사전/캐시/AI)
│       ├── FigmaScreenshots    /v1/images 로 프레임 PNG (선택)
│       └── FigmaAiConversionService  AI 경로 오케스트레이션 (Spring @Service)
...
### 9.1 v2.0 — 템플릿 기반 변환 (2026-09-19)

**분석 결과(v1.x 의 문제)**
- 좌표 복사 방식이라 표준 화면 구조가 나오지 않음. 컨트롤 판별이 레이어 이름 문자열에 의존(버튼 캡션이 대부분 `Button`).
...
| `exconverter/*` (14개) | eXConverter-AI 템플릿 엔진 이식 + §8.1 변경 |
...
**삭제** — `service/AdvancedFigmaToClxService`, `util/ClxLayoutUtil`(폼 레이아웃 실험 코드, 템플릿 파이프라인이 대체. git 이력에 남아 있음).
...
### 9.2 v2.1 — AI 보조 경로 (2026-09-20)

무료 구성만 검토해 **"규칙이 뼈대, AI 는 이견 제시"** 구조를 새로 추가했다. 기존 기능·엔드포인트는 건드리지 않았다.

| 추가 파일 | 내용 |
|---|---|
| `docs/ai-architecture.md` | 후보 구성 5가지 비교와 채택 근거, 안전장치, 비용/한도, 검증 결과 |
| `figma/ai/AiClient`, `AiClients`, `GeminiAiClient`, `OllamaAiClient` | 제공자 추상화(none/gemini/ollama), 429·키없음·타임아웃은 폴백 |
| `figma/ai/UiIrPatch` | 허용 op 화이트리스트 + 인덱스/값 검증 + 적용, 거부 사유 기록 |
| `figma/ai/UiIrCritic` | 구조 검토·수리 프롬프트, 적용 후 UI-IR 재파싱 실패 시 폐기 |
| `figma/ai/LabelCodeNamer` | 라벨 → 컬럼코드(사전 → 캐시 → AI), `generated/ai-cache/label-codes.json` |
| `figma/ai/FigmaScreenshots` | `/v1/images` 로 프레임 PNG(선택, 긴 변 1024px) |
| `figma/ai/FigmaAiConversionService` | 추출 → 이름 → 검토 → 생성 → 수리(최대 2회) → 규칙 결과 복귀 |
| `web/FigmaAiController` | `/designAi/convert.do`, `preview.do`, `jsonConvert.do`, `status.do` |
...
| 한글이 깨진 응답 | 응답은 `text/plain;charset=UTF-8`. 구버전 배포본인지 확인(Publish/재시작) |
...
1. CLX XML 을 문자열로 직접 쓰지 말 것. 새 변환 규칙은 **UI-IR 추출(`FigmaUiIrExtractor`)** 에, 화면 구조 규칙은 **생성기/템플릿** 에 넣는다.
   AI 에게 CLX 를 만들게 하지 말 것(§3.3). AI 출력은 허용된 UI-IR patch 뿐이며, 새 op 를 추가하면 `UiIrPatch` 검증과 `FigmaAiHarness` 시나리오도 함께 추가한다.
...
5. AI 경로를 바꾸면 `FigmaAiHarness`(7/7) 를 돌리고, **AI 를 끈 상태의 결과가 변하지 않는지** 함께 확인한다.
